...
import { getUser } from '~/loader-functions/user.server';
import { login, register } from '~/utils/auth.server';
import { errorResponse } from '~/utils/responses';
import { validateEmail, validatePassword, validateUsername } from '~/utils/validators.server';
  ...
  return (
    <Layout>
      <div className="h-full flex justify-center items-center flex-col gap-y-4">
        <button onClick={() => setAction(action === Action.Login ? Action.SignUp : Action.Login)} className="absolute top-8 right-8 rounded-xl bg-black font-semibold text-red-400 px-3 py-2">
          {action === Action.Login ? 'Sign Up' : 'Sign In'}
        </button>
        <h2 className="text-3xl font-bold">Formula 1 Predictions</h2>
        <p className="font-semibold text-slate-300">{action === Action.Login ? 'Log In To Play' : 'Sign Up To Get Started!'}</p>

        <form method="POST" className="mt-2 rounded-2xl bg-black-200 p-6 w-96 bg-gray-900 border-red-500 border-2">
          {formError && <div className="text-xs font-semibold text-center tracking-wide text-red-500 w-full">{formError}</div>}

          <FormField htmlFor="email" label="Email" value={formData.email} onChange={(e) => handleInputChange(e, 'email')} error={errors?.email} />

          {action !== Action.Login ? <FormField htmlFor="username" label="Username" value={formData.username} onChange={(e) => handleInputChange(e, 'username')} error={errors?.username} /> : null}
          ...
          <div className="w-full text-center mt-2">
            <button type="submit" name="_action" value={action} className="rounded-xl font-semibold text-red-400 px-3 py-2">
